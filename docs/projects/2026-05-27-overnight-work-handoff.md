...
- [ ] 0. `feature/overnight-2026-05-27` 브랜치 생성 —
- [ ] B. 회귀 테스트 — (파일: / 테스트: / Codex: )
- [ ] PoC. C/E inlier 측정 — C=__ E=__ → 선택=__
- [ ] D. 종료감지 OCR 필수화 — (파일: / 테스트: / **라이브확인 필요**)
- [ ] 4. 기능 구현(C 또는 E) — (파일: / 테스트: / **라이브확인 필요**)
- [ ] 5. 어드민 페이지(env 비번) — (파일: / **라이브확인 필요**)

## 8. 막힌 점 / 결정 대기 (사람용)
- (없음 — 생기면 항목·이유·필요한 결정을 적어라)

## 9. 아침 보고용 최종 요약 (큐 종료 시 작성)
> 아침에 사용자가 새 세션에서 "이전 작업 결과 알려줘"라고 하면, 세션은 §7·§9를 읽고 이 형식으로 보고한다.
- **완료한 것**:
- **미완/막힌 것**:
- **라이브 검증 대기**(아침에 화면공유로 확인할 항목):
- **브랜치/커밋**: `feature/overnight-2026-05-27` (커밋 N개, 머지 전 리뷰 필요)
- **다음 추천 단계**:
