...
1. 실제 게임 페이즈 1 실측 (도커 재빌드 후) — ✓ 완료, false positive도 사라짐 확인
2. 페이즈 2~9 stateless 통합 RANSAC — ✓ 완료 (사용자 게임 실측 검증 대기)
3. 격자선 기반 확대 비율 검출 — ✓ 완료 (검증 사진 없어 알고리즘만 박음)
4. 페이즈 9 특수 처리 — 점 크기, 점수 임계 12점 floor 적용
5. 확대 + 패닝 풀 풀이 — 도시명 OCR 또는 ORB 매칭 (별도 작업)
6. OCR 페이즈 번호 직접 추출 (게임 화면 확인 후)
